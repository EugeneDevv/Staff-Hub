...
import { useLazySingleUserQuery, useUpdateProfileMutation } from '../../../application/services/authApi';
...
import { certifiCationToDTO, certificationHasData, educationToDTO, experienceToDTO, mapCertificationPayloadListToCertificationList, mapEducationPayloadListToEducationList, mapExperiencePayloadListToExperienceList, mapProjectPayloadListToProjectDataList, projectDataToDTO, projectHasData, } from '../../../domain/valueObjects/utils';
...
const CompleteProfilePage = () => {
  ...
  const tempUpdateMyProfile = localStorage.getItem('updateMyProfile') ?? 'true';

  const updateMyProfile = tempUpdateMyProfile === 'true';
  ...
  const mappedEducations = mapEducationPayloadListToEducationList(user?.educations ?? []);
  const mappedExperiences = mapExperiencePayloadListToExperienceList(user?.experiences ?? []);
  const mappedCertifications = mapCertificationPayloadListToCertificationList(user?.certifications ?? []);
  const mappedProjects = mapProjectPayloadListToProjectDataList(user?.userProjects ?? []);
  const userSkills: SkillPayload[] = user?.userSkills ?? [];
  const [educationDetails, setEducationDetails] = useState<Education[]>(mappedEducations.length > 0 ? mappedEducations : initialEducationListState);
  const [experienceDetails, setExperienceDetails] = useState<Experience[]>(mappedExperiences.length > 0 ? mappedExperiences : initialExperienceState);
  const [certifications, setCertifications] = useState<Certification[]>(mappedCertifications.length > 0 ? mappedCertifications : initialCertificationListState);
  const [skills, setSKills] = useState<SkillPayload[]>(userSkills.length > 0 ? userSkills : initialSkillListState);

  const [projects, setProjects] = useState<ProjectData[]>(mappedProjects.length > 0 ? mappedProjects : initialProjectsState);
  ...
  }, [isSuccess, submitted, fetchUserProfile, updateMyProfile, navigate])

  return (
    <div className="completeProfileWrapper">
      <header className="navbar"><h2>G.E.R</h2>
        <h6>{updateMyProfile ? 'Update your profile below' : 'Update employee profile below'}</h6></header>
      <form className="content" onSubmit={handleSubmit}>
        ...
        <div className="profileDetail">
          ...
          {isLoading && <Spinner />}
          {!isLoading && <SubmitButton value='Save' disabled={false} />}
        </div>
